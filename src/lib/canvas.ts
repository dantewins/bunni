        ...
        const upsertAssignment = async (item: any) => {
            ...
            const prompt = `
                ...
                Due at: ${ass.due_at || "no due date"}
                {
                    ...
                    "name": "Nicely formatted name, meaning add spaces where appropriate and remove dates in names (e.g., 'Point of View Notes', 'HW #10 6.13 Improper Integrals', 'Unit 2 Progress Check MCQ', 'AP Video 3.1 - 3.3 Summaries + Quiz')"
                }
            `;
